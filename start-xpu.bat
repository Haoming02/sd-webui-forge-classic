...
set "COMMANDLINE_ARGS=--port 7863 --ui-config-file ui-config-clean.json --gpu-only --reserve-vram 1.0 --skip-install --skip-torch-cuda-test --gpu-device-id 0 --bf16-unet --bf16-vae --fp16-text-enc --disable-ipex-optimize --skip-prepare-environment --no-hashing --use-pytorch-cross-attention --disable-xformers --disable-sage --disable-flash --model-ref C:\\Users\\Derek\\models"
