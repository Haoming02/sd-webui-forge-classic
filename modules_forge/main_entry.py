...
# Locale codes that should never appear as standalone dropdown choices.
# en_US.json (identity map) duplicates the "None" default — having both in
# the menu is confusing, so the file is generated by build_translations
# for completeness but hidden from the live selector.
HIDDEN_LANGUAGES: set[str] = {"en_US"}


def make_language_selector_ui():
    """Top-bar language dropdown wired to shared.opts.localization.

    Lives in the quicksettings row, far right (CSS `margin-left: auto` on its
    `elem_id`). Selecting a value writes the new localization to the user
    config and triggers a full UI restart via the same `restart_reload`
    mechanism the existing "Restart UI" button uses (see ui_settings.py).

    Choices use (display_label, locale_code) tuples so the user sees the ISO
    code + autoglottonym (e.g. "IT  ·  Italiano") while shared.opts.localization
    stays set to the file-stem locale code (e.g. "it_IT"). The labels are
    plain ASCII + native script - no flag emojis - because regional indicator
    flag glyphs render inconsistently across browsers/fonts on Windows.
    """
    global ui_language

    def current_choices():
        codes = ["None"]
        for k in localization.localizations.keys():
            if k in HIDDEN_LANGUAGES:
                continue
            codes.append(k)
    ...
    def refresh_choices():
        ...
        return gr.update(choices=current_choices())

    ui_language = gr.Dropdown(
        label="Language",
        value=lambda: shared.opts.localization or "None",
    ...
    def apply_language(value: str) -> None:
        chosen = value if value else "None"
        if shared.opts.data.get("localization") != chosen:
            shared.opts.set("localization", chosen)
            shared.opts.save(shared.config_filename)
        shared.state.request_restart()

    ui_language.change(
        fn=apply_language,
        _js="(v) => { restart_reload(); return v; }",
        inputs=[ui_language],
        outputs=[],
        queue=False,
        show_progress=False,
    ...
    Context.root_block.load(fn=refresh_choices, outputs=[ui_language], queue=False)
